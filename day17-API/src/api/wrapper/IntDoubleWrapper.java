...
/**
 * 기본형 <==> 잠초형 테이터를 서로 변환할 수 있도록 하는 포장클래스중
 * 
 * int 	  : Integer
 * double : Double
 * 
...
public class IntDoubleWrapper {
	...
	public static void main(String[] args) {
		...
		// 클래스 이름 생략
		intFrm = String.format("문자열[%s] ==> 기본형(int)[%d]", intInput, parseInt(intInput));
				
		dbFrm = String.format("문자열[%s] ==> 기본형(double)[%f]", dbInput,  parseDouble(dbInput));
		...
		//                             인스턴스 생성 후 사용
		intFrm = String.format("문자열[%s] ==> 기본형(int)[%d]", intInput, new Integer(intInput).intValue());
