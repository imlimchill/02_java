...
public class PointTest2 {
	...
	public static void main(String[] args) {
		// 1. Point 배열 선언, 2. 초기화
		Point[] points = {new Point(10,0), new Point(3, 4), new NamedPoint("0", 0, 0)};
		...
		for (Point point: points) {
			...
			// 1. toString() 메소드는 print() 계열 메소드에서 객체 참조 변수를 출력할 떄
			//    자동으로 작동하는 특징을 가지고 있다.
			
			// 2. toString() 은 Object 에서 물려받은 메소드로 모든 서브클래스들이 가지고 있으며
			//    재정의된 경우에는 해당 인스턴스 타입에 맞추어 부모클래스에 정의된 메소드로 호출을 해도 
			//    인스턴스 타입의 재정의된 메소드가 자동으로 응답
			//    => 가상 메모리 호출 (virtual method invocation)
		}
