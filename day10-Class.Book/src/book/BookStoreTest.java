...
/**
 * 서점에서 할 수 있는 여러 기능들
 * 책찾기, 책 수정, 신간추가, 책페기 등의 기능을 케스트 하는 클래스
...
public class BookStoreTest {
	...
	public static void main(String[] args) {
		...
		// 1. 매내저에게 전달할 책장안에 들어가는 배열
		Book[] books = new Book[0];
		
		// 2. 배열로 구성되는 책장을 만든다.
		BookShelf bookShelf = new BookShelf(books);
		
		// 3. 배열로 구성된 책장을 관리할 매너지를 고용
		...
		// ------------------------------------------------------------
		// 1. 서점에 신간이 도착
		System.out.println("== 신간도착 책 생성 ==");
		Book vegetarian = new Book(1, "9788936433598", "채식주의자", "한강", "창비", 247, 10800, 10);
		Book littlePrince = new Book(2, "9791159039690", "어린완자(초판본)", "생텍쥐페리", "더스토리", 150, 8820, 5);
		...
		// 4. 채식주의자 가격할인 : set
		System.out.println("== 채식주의자 정보변경 ==");
		// (1) 가격 할인된 채식주의자 책 정보를 새로 생성
		Book vegetarian2 = new Book(1, "9788936433598", "채식주의자", "한강", "창비", 247, 10000, 7);
		
		// (2) 변경된 책 정보를 담은 책 객체를 넘겨서 수정처리
		kyobo.set(vegetarian2);
		
		// 5. 채식주의자 할인 적용 결과 확인 : get
		// (1) 해당 책의 시퀀스 번호만 알면 조회가능 > 시쿼스 번호만 들어있는 Book 객체 생상
		...
		// 7. 폐기 확인을 위해 전체 목록 재조회 : getAllBooks
		System.out.println("== 삭제 후 전체 목록 확인 ==");
